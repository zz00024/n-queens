...
window.findNQueensSolution = function(n) {
  ...
  //add one more search condition which is checking major diagonal&minor diagonal conflict
  var solution = new Board({n: n}); //fixme
  var board = solution.rows();
  // create a board
  var col = n;
  ...
  for (var i = 0; i < row; i++) {
    for (var j = 0; j < col; j++) {
      board[i][j] = 1;
      //add another checking condition here for diagonal conflict
      if (solution.hasRowConflictAt(i) || solution.hasColConflictAt(j) || solution.hasMajorDiagonalConflictAt(i) || solution.hasMinorDiagonalConflictAt(i)) {
        board[i][j] = 0;
      }
      console.log(board);
    }
